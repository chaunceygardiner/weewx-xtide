...
class TestAmericanEnglish(unittest.TestCase):
    ...
    # clean tree.
    A_TRACKED_FILE = 'bin/user/xtide.py'

    # PER-REPO.
    NOT_OURS = (
        'LICENSE',
        # Nothing else: this repo checks in no third-party text.  Its
        # tests drive fake `tide` executables written at run time rather
        # than recorded fixtures, so every other tracked byte is ours.
    )
    ...
    # arrives.
    LANG_DIR = 'skins/xtide/lang/'
    LANG_IN_ENGLISH = 'skins/xtide/lang/en.conf'
